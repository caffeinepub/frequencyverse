...
const BackgroundAmbianceContext = createContext<
  BackgroundAmbianceContextType | undefined
>(undefined);

const AMBIANCE_STORAGE_KEY = "frequencyverse-background-ambiance";

export function BackgroundAmbianceProvider({
  children,
}: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    console.log(
      "🎵 [AMBIANCE] Initializing audio for:",
      currentAmbiance,
      "URL:",
      assetUrl,
    );
    ...
    try {
      ...
      const attemptPlay = () => {
        ...
        if (player.isPlaying) {
          console.log(
            "🎵 [AMBIANCE] Main player active, keeping ambiance silent",
          );
        ...
        audio
          ...
          .catch((e) => {
            console.warn(
              "⚠️ [AMBIANCE] Play failed (may need user interaction):",
              e.message,
            );
            pendingPlayRef.current = true;
          });
  ...
  }, [currentAmbiance, isAudioActivated, player.isPlaying]);
  ...
  useEffect(() => {
    ...
    if (player.isPlaying) {
      ...
      if (
        wasInterruptedByMainPlayerRef.current &&
        isAudioActivated &&
        audio.paused
      ) {
  ...
  return (
    <BackgroundAmbianceContext.Provider
      value={{ currentAmbiance, setAmbiance, isPlaying }}
    >
      {children}
    </BackgroundAmbianceContext.Provider>
  );
...
export function useBackgroundAmbiance() {
  ...
  if (context === undefined) {
    throw new Error(
      "useBackgroundAmbiance must be used within a BackgroundAmbianceProvider",
    );
  }
  return context;
}
