...
export function useUnifiedSessionManager() {
  ...
  const startSession = useCallback(
    (session: MeditationSession) => {
      ...
      const durationPerFreq = Math.floor(
        totalSeconds / session.frequencies.length,
      );
      ...
      timerRef.current = setInterval(() => {
        player.setTimeRemaining((prev) => {
          ...
          if (
            newIndex !== player.currentIndex &&
            newIndex < session.frequencies.length
          ) {
  ...
  const resumeSession = useCallback(() => {
    ...
    const durationPerFreq = Math.floor(
      totalSeconds / activeSession.frequencies.length,
    );
    ...
    timerRef.current = setInterval(() => {
      player.setTimeRemaining((prev) => {
        ...
        if (
          newIndex !== player.currentIndex &&
          newIndex < activeSession.frequencies.length
        ) {
          player.updateCurrentIndex(newIndex);
        }
