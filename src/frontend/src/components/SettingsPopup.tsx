...
export default function SettingsPopup({ onClose }: SettingsPopupProps) {
  ...
  useEffect(() => {
    ...
    const calculatePosition = () => {
      ...
      // Find the settings button in the header - search for all possible settings labels across languages
      const settingsButton = document.querySelector('button[aria-label*="Settings"], button[aria-label*="Ayarlar"], button[aria-label*="Configuración"], button[aria-label*="Paramètres"], button[aria-label*="Einstellungen"], button[aria-label*="Impostazioni"], button[aria-label*="Настройки"], button[aria-label*="الإعدادات"], button[aria-label*="設定"], button[aria-label*="设置"]');
