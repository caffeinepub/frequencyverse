...
type AnimationIntensityLevel = "low" | "medium" | "high";
...
interface PlayerAnimationOverlayProps {
  ...
  animationIntensity?: AnimationIntensityLevel;
}
...
export default function PlayerAnimationOverlay({
  ...
  useEffect(() => {
    ...
    // Apply animation intensity multiplier to particle count
    const intensityMultiplier =
      animationIntensity === "low"
        ...
        : animationIntensity === "high"
          ? 1.5
          : 1.0;
    const adjustedParticleCount = Math.round(
      config.particleCount * intensityMultiplier,
    );
    const intensityAlphaScale = animationIntensity === "low" ? 0.6 : 1.0;
    ...
    for (let i = 0; i < adjustedParticleCount; i++) {
      particles.push({
        angle: (Math.PI * 2 * i) / adjustedParticleCount,
        radius: 100 + Math.random() * 150,
        speed: 0.006 + Math.random() * 0.012,
        size: Math.random() * 2.5 + 1.2,
        hue: Math.random() > 0.5 ? config.baseHue : config.secondaryHue,
        alpha: (Math.random() * 0.3 + 0.4) * intensityAlphaScale,
    ...
    // Lifecycle management: pause animation when document is hidden (app backgrounded)
    // This prevents BLASTBufferQueue errors on Android and saves battery
    const handleVisibilityChange = () => {
      if (document.hidden) {
        // App went to background - stop animation loop
        ...
        // App came back to foreground - restart animation loop if overlay is open
    ...
    // pagehide/pageshow for older Android WebViews that don't fire visibilitychange reliably
