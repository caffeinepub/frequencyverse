...
export default function MainPlayer() {
  ...
  const [volume, setVolumeState] = useState(
    Math.round(audioManager.volume * 100),
  );
  ...
  const getCurrentItemName = () => {
    ...
    if (player.currentSoundId) {
      const kidsSound = kidsSleepSounds.find(
        (s) => s.id === player.currentSoundId,
      );
      if (kidsSound) {
        return t.kidsSleepSounds[kidsSound.category][kidsSound.title];
      }
      const peacefulSound = peacefulSounds.find(
        (s) => s.id === player.currentSoundId,
      );
      if (peacefulSound) {
  ...
  const hasActiveAudio =
    player.currentFrequency !== null || player.currentSoundId !== null;
  ...
  return (
    <>
      <div
        className={`fixed bottom-0 left-0 right-0 bg-gradient-to-r ${themeStyles.gradient} backdrop-blur-xl border-t ${themeStyles.border} ${themeStyles.glow} z-50 transition-all duration-500`}
      >
          ...
          <div className="px-3 sm:px-4 py-3">
            <div className="flex flex-col gap-3">
              ...
              <div className="flex-1 min-w-0 text-center md:text-left">
                <div className="text-white font-semibold text-sm sm:text-base truncate drop-shadow-lg">
                  {getCurrentItemName()}
                </div>
                {player.sessionName && (
                  <div className="text-white/70 text-xs sm:text-sm truncate drop-shadow-md">
                    {player.sessionName}
                  </div>
                )}
              ...
              <div className="flex items-center justify-center gap-1 sm:gap-2 flex-wrap">
                ...
                >
                  ...
                  <span className="text-xs sm:text-sm font-medium whitespace-nowrap">
                    {player.selectedDuration} {t.durationUnit}
                  </span>
                </Button>
