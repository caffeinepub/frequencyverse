...
export default function BackgroundAmbianceSelectorPopup({ onClose }: BackgroundAmbianceSelectorPopupProps) {
  ...
  useEffect(() => {
    ...
    const calculatePosition = () => {
      ...
      const ambianceButton = document.querySelector('button[aria-label*="Background Sounds"], button[aria-label*="Arka Plan Sesleri"], button[aria-label*="Sonidos de Fondo"], button[aria-label*="Sons de Fond"], button[aria-label*="Hintergrundgeräusche"], button[aria-label*="Suoni di Sottofondo"], button[aria-label*="Фоновые Звуки"], button[aria-label*="أصوات الخلفية"], button[aria-label*="背景音"], button[aria-label*="背景声音"], button[aria-label*="Sons de Fundo"]');
